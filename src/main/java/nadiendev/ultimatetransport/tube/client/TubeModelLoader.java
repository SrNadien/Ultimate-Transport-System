...
/**
 * The two custom models are plain {@code {"loader": "ultimatetransport:tube"}} / {@code {"loader": "ultimatetransport:station"}}
...
public final class TubeModelLoader implements IUnbakedGeometry<TubeModelLoader> {
    ...
    public static List<String> spriteNames() {
        ...
        for (int i = 0; i < 16; i++) {
            names.add("ultimatetransport:block/tube/" + i);
        }
        for (int c = 0; c < 6; c++) {
            for (int i = 0; i < 16; i++) {
                names.add("ultimatetransport:block/tube" + c + "/" + i);
            }
        }
        names.add("ultimatetransport:block/station_misc");
        for (int i = 1; i <= 4; i++) {
            names.add("ultimatetransport:block/station_side" + i);
            names.add("ultimatetransport:block/station_entr" + i);
        }
        return names;
    }
